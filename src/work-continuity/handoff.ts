...
function hasUnfinishedWork(state: WorkState): boolean {
  return (
    state.phases.some(
      (item) =>
...
export class SmartHandoffManager {
  ...
  async capture(
    ...
    const manual = loadProjectManual(
      this.options.project,
     ...
     */
    const id = sha256(
      [this.options.project.id, this.options.identity.sessionKey, stateDigest].join('|')
    ).slice(0, 24);

    const createdAt = new Date().toISOString();

    const handoff: SmartHandoff = {
      version: 1,
      ...
      sourceSession: {
        ...
        nativeSessionId: this.options.identity.nativeSessionId,

        sessionKey: this.options.identity.sessionKey,
