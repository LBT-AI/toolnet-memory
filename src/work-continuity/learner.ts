...
import { extractWorkObservations } from './extractor.js';

import { WorkObservationJournal } from './journal.js';

import { reconcileWorkState } from './reducer.js';

import type { WorkContinuityResult } from './types.js';
...
export class WorkContinuityLearner {
  private readonly journal: WorkObservationJournal;
  ...
  constructor(
    ...
    this.journal = new WorkObservationJournal(options.storage);
  }

  async learnNew(): Promise<WorkContinuityResult> {
    const state = this.options.wal.loadState();

    const rawOffset = Number(state.sourceCursors['work.continuity.offset'] ?? 0);
    ...
    const observations = extractWorkObservations(
    ...
    this.options.wal.setSourceCursor('work.continuity.offset', read.nextOffset);
