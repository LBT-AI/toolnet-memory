...
> {
  ...
  if (
    ...
    manualContent =
      await options.storage
        .getText(
          `projects/${options.project.id}/project/manual.md`,
        ) ??
      "";
  }
  ...
  if (
    ...
    if (
      semantic.activeObjective
    ) {
      lines.push(
        "",
        "CURRENT OBJECTIVE",
        clip(
          semantic.activeObjective.value,
          420,
        ),
      );
    }
    ...
    if (
      semantic.planRationale
    ) {
      lines.push(
        "",
        "WHY THIS APPROACH",
        clip(
          semantic.planRationale.value,
          420,
        ),
      );
    }
  ...
  if (
    ...
    if (
      ...
      lines.push(
        `Current phase: ${state.currentPhase.title} [${state.currentPhase.status}]`,
      );
    }
    ...
    if (
      ...
      if (
        ...
        if (
          semanticPhase.objective
        ) {
          lines.push(
            `Phase objective: ${clip(semanticPhase.objective.value, 340)}`,
          );
        }
        ...
        if (
          ...
          lines.push(
            `Why this phase: ${clip(semanticPhase.why.value, 340)}`,
          );
        } else {
          lines.push(
        ...
        if (
          semanticPhase.deliverable
        ) {
          lines.push(
            `Deliverable: ${clip(semanticPhase.deliverable.value, 340)}`,
          );
        }

        if (
          semanticPhase.dependencies.length
        ) {
          lines.push(
        ...
        if (
          semanticPhase.acceptanceCriteria.length
        ) {
          lines.push(
            "",
            "DEFINITION OF DONE",
          ...
          semanticPhase.acceptanceCriteria
            .slice(
              0,
              6,
            )
        ...
        if (
          semanticPhase.openQuestions.length
        ) {
          lines.push(
            "",
    ...
    if (
      ...
      lines.push(
        `Current task: ${state.currentTask.title} [${state.currentTask.status}]`,
      );
    }
  ...
  if (
    ...
    lines.push(
      `Latest handoff: ${handoff.reason} / ${handoff.sourceSession.agent}`,
    );
  }
