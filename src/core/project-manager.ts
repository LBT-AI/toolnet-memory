 ...
 * depends on the path.
 */
function legacyProjectIdFromPath(
  rootPath: string,
): string {
...
function parseManifest(
  ...
  if (
    ...
    throw new Error(
      `ToolNet project manifest is missing id: ${filePath}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `ToolNet project manifest is missing name: ${filePath}`,
    );
  }
  ...
  return {
    ...
    graphVersion:
      typeof data.graphVersion ===
        "number"
        ? data.graphVersion
        : 0,

    memoryVersion:
      typeof data.memoryVersion ===
        "number"
        ? data.memoryVersion
        : 0,
