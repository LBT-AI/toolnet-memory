...
// Everything this worker writes is transient — erased by the next
// frame or by the parent's phase-done line — so ASCII here never shows up
// in scrollback; the persistent lines are printed by the parent
...
// colors:false (NO_COLOR / --no-color on an interactive TTY) keeps the
...
export interface ShimmerWorkerMessage {
  type: 'update' | 'stop';
  phaseName?: string;
  percent?: number;
  count?: number;
...
function render(): void {
  ...
  let line: string;
  if (currentPercent >= 0) {
    // Real progress: show progress bar with %
    const barWidth = 25;
    const filled = Math.round((barWidth * currentPercent) / 100);
    const empty = barWidth - filled;
    line = `${DM}${G.rail}${RST}  ${color}${glyph}${RST} ${currentMessage}  ${renderBar(
      frame,
      filled,
      empty
    )} ${currentPercent}%`;
  } else if (currentCount > 0) {
    // Scan phase: show count without %
    line = `${DM}${G.rail}${RST}  ${color}${glyph}${RST} ${currentMessage}... ${formatNumber(
      currentCount
    )} found`;
  } else {
    // Shimmer-only: no measurable progress, just animation
...
// this worker's raw-byte path must never leave bytes in scrollback.
...
parentPort!.on('message', (msg: ShimmerWorkerMessage) => {
  if (msg.type === 'update') {
    ...
    currentPercent = msg.percent ?? -1;
    currentCount = msg.count ?? 0;
