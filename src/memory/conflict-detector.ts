...
export class ConflictDetector {
  findSuperseded(next: MemoryRecord, existing: MemoryRecord[]): MemoryRecord[] {
    ...
    const nextTopic = typeof next.metadata?.topic === 'string' ? next.metadata.topic : undefined;

    const nextTags = meaningfulTags(next);
    ...
    return existing.filter((old) => {
      ...
      const oldTopic = typeof old.metadata?.topic === 'string' ? old.metadata.topic : undefined;

      if (nextTopic && oldTopic && nextTopic === oldTopic) {
        return true;
      }

      const oldTags = meaningfulTags(old);
