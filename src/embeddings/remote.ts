...
export class HuggingFaceEmbeddingProvider implements EmbeddingProvider {
  ...
  async embedMany(texts: string[]): Promise<number[][]> {
    ...
    const url = `https://router.huggingface.co/hf-inference/models/${this.options.model}/pipeline/feature-extraction`;
    ...
    if (!response.ok) {
      throw new Error(`HF embedding failed: ${response.status} ${await response.text()}`);
    }

    const data = await response.json();

    if (!Array.isArray(data)) {
      throw new Error('HF embedding returned invalid response');
    }
     ...
     */
    return data.map((item: unknown) => normalizeEmbedding(item));
  }
}
