...
export class IncrementalRepositoryIndexer {
  ...
  async index(projectId: string, rootPath: string): Promise<IncrementalIndexResult> {
    ...
    // Nothing changed.
    if (previousManifest && previousGraph && changed.length === 0 && diff.deleted.length === 0) {
      return {
        firstRun: false,
    ...
    const preservedSymbols =
      previousGraph?.symbols.filter((symbol) => diff.unchanged.includes(symbol.filePath)) ?? [];

    const parsed = [];

    for (const filePath of changed) {
      parsed.push(await parseTypeScriptFile(projectId, rootPath, filePath));
    }

    /*
     * Build graph for changed files.
     */
    const changedGraph = new GraphBuilder().build(projectId, parsed);
     ...
     */
    const finalGraph = new CodeGraphStore();

    for (const symbol of preservedSymbols) {
      finalGraph.addSymbol(symbol);
    }

    for (const symbol of changedGraph.allSymbols(projectId)) {
      finalGraph.addSymbol(symbol);
    }

    const validIds = new Set(finalGraph.allSymbols(projectId).map((symbol) => symbol.id));
    ...
    for (const edge of previousGraph?.edges ?? []) {
      if (
        ...
        !changed.includes(
          previousGraph!.symbols.find((symbol) => symbol.id === edge.from)?.filePath ?? ''
        ) &&
        !changed.includes(
          previousGraph!.symbols.find((symbol) => symbol.id === edge.to)?.filePath ?? ''
        )
      ) {
    ...
    for (const edge of changedGraph.allEdges(projectId)) {
      finalGraph.addEdge(edge);
    }
