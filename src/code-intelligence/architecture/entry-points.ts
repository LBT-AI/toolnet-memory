...
import type { ArchitectureEntryPoint, EntryPointKind } from './types.js';
...
export class EntryPointDetector {
  ...
  detect(projectId: string): ArchitectureEntryPoint[] {
    ...
    for (const symbol of symbols) {
       ...
       */
      if (symbol.type !== 'route' && symbol.type !== 'file') {
        continue;
      }
     ...
     */
    const bestByKey = new Map<string, ArchitectureEntryPoint>();

    for (const item of results) {
      const key = item.kind === 'route' ? item.symbolId : item.filePath;
  ...
  private classify(
    ...
    let kind: EntryPointKind | null = null;
    ...
    if (path.startsWith('bin/')) {
    ...
    } else if (/(^|\/)(main|server)\.[^.]+$/.test(path)) {
      kind = 'main';

      score = 90;

      reasons.push('main/server file');
    } else if (path.includes('/runtime/') && /(^|\/)(bootstrap|index)\.[^.]+$/.test(path)) {
      kind = 'runtime';
      ...
      const incoming = edges.filter(
        (edge) => edge.to === symbol.id && edge.type !== 'DEFINES'
      ).length;

      const outgoing = edges.filter(
        (edge) => edge.from === symbol.id && edge.type !== 'DEFINES'
      ).length;

      if (path.startsWith('packages/') || incoming === 0 || outgoing > 0) {
        kind = 'package';

        score = path.startsWith('packages/') ? 70 : 55;
