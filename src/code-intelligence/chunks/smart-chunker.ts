...
export class SmartCodeChunker {
  ...
  async build(
    ...
    for (const [filePath, fileSymbols] of byFile) {
      ...
      try {
        text = await readFile(join(rootPath, filePath), 'utf8');
      } catch {
        continue;
      }
  ...
  private chunkRange(input: {
    ...
    const { projectId, filePath, lines, symbol, maxLines } = input;

    const overlapLines = Math.min(input.overlapLines, Math.max(0, maxLines - 1));

    const start = Math.max(1, input.startLine);

    const end = Math.min(lines.length, Math.max(start, input.endLine));

    const output: CodeChunk[] = [];
    ...
    while (cursor <= end) {
      const chunkEnd = Math.min(end, cursor + maxLines - 1);

      const raw = lines
        .slice(cursor - 1, chunkEnd)
        .join('\n')
        .trim();

      if (raw) {
        const content = this.sanitizer.sanitize(raw).text;
      ...
      cursor = chunkEnd - overlapLines + 1;
