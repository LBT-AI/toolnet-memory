...
export class TypeScriptTypeResolver {
  ...
  async resolveProject(
    ...
    return {
      ...
      exact: output.filter((item) => item.confidence === 'exact').length,

      high: output.filter((item) => item.confidence === 'high').length,

      fallback: output.filter((item) => item.confidence === 'fallback').length,

      resolutions: output,
    };
  }

  private resolveCall(input: {
    projectId: string;
    rootPath: string;
    ...
    const { projectId, rootPath, source, node, checker } = input;
    ...
    if (!declaration) {
      return this.fallbackCall(projectId, rootPath, source, node);
    }
  ...
  private resolveHeritage(input: {
    ...
    for (const clause of node.heritageClauses ?? []) {
      for (const item of clause.types) {
        const symbol = checker.getSymbolAtLocation(item.expression);
        ...
        const result = this.fromDeclaration({
          projectId,
          rootPath,
          source,
  ...
  private fromDeclaration(input: {
    ...
    kind: TypeResolution['kind'];
  }): TypeResolution | null {
    const declarationSource = input.declaration.getSourceFile();

    if (declarationSource.isDeclarationFile) {
      return null;
    }

    const targetFile = normalize(relative(input.rootPath, resolve(declarationSource.fileName)));

    if (targetFile.startsWith('../') || targetFile.includes('node_modules/')) {
      return null;
    }

    const targetLine = lineOf(declarationSource, input.declaration);

    const targetName = declarationName(input.declaration) ?? input.expression.split('.').at(-1);

    const graphSymbol = this.findGraphSymbol(input.projectId, targetFile, targetLine, targetName);

    const sourceFile = normalize(relative(input.rootPath, resolve(input.source.fileName)));
    ...
    return {
      id: hash(
        [
          input.projectId,
          sourceFile,
          lineOf(input.source, input.sourceNode),
          input.kind,
          targetFile,
          targetLine,
        ].join(':')
      ...
      sourceLine: lineOf(input.source, input.sourceNode),
      ...
      targetQualifiedName: graphSymbol?.qualifiedName,

      targetSymbolId: graphSymbol?.id,

      confidence: graphSymbol ? 'exact' : 'high',
  ...
  private fallbackCall(
    ...
    return {
      id: hash(`${projectId}:${sourceFile}:${lineOf(source, node)}:${target.id}`),
  ...
  private findGraphSymbol(
    ...
    const symbols = this.graph
      ...
      .filter((symbol) => normalize(symbol.filePath) === normalize(filePath));
