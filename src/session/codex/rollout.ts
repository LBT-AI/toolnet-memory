...
function classify(row: Record<string, unknown>): {
  ...
  const outerType = typeof row.type === 'string' ? row.type.toLowerCase() : '';
  ...
  if (
    ...
    payloadType.includes('exec_command_begin') ||
    payloadType.includes('exec_command_start')
  ) {
    return {
      type: 'tool_call',
    };
  ...
  if (
    ...
    payloadType.includes('tool_result') ||
    payloadType.includes('exec_command_end') ||
    payloadType.includes('exec_command_complete')
  ) {
    return {
      type: 'tool_result',
    };
  ...
  if (outerType.includes('compacted') || payloadType.includes('compacted')) {
    return {
...
export function readCodexRollout(
  ...
  for (const line of complete.split('\n')) {
    ...
    const absoluteOffset = start + localOffset;
    ...
    events.push({
      ...
      sourceEventId: `rollout:${absoluteOffset}`,
