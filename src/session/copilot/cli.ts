import { installCopilotIntegration } from './installer.js';

import { inspectCopilotScopedIntegrationStatus } from './scoped-status.js';
...
function renderStatus(scope: IntegrationScope, projectRoot?: string): void {
  ...
  console.log('GitHub Copilot CLI Integration');
  console.log('==============================');
  console.log('');
  ...
  if (status.projectScope) {
    ...
    console.log(
      `  Instruction : ${status.projectScope.instruction.configured ? 'ready' : 'missing'} — ${status.projectScope.instruction.instructionFile}`
    );
...
function main(): void {
  ...
  if (!status.installed) {
    throw new Error(
      `GitHub Copilot CLI integration installation did not verify successfully (state=${status.state}).`
    );
  }
  ...
  console.log('✅ GitHub Copilot CLI integration installed');
  ...
  if (scope === 'both') {
    ...
    console.log('Project MCP precedence: project > global');
    console.log('Project trust: required by Copilot CLI');
  }
