import { installGrokIntegration } from './installer.js';

import { inspectGrokScopedIntegrationStatus } from './scoped-status.js';
...
function renderStatus(scope: IntegrationScope, projectRoot?: string): void {
  ...
  console.log('Grok Build Integration');
  console.log('======================');
  console.log('');
  ...
  if (status.global) {
    ...
    console.log(
      `  Skill : ${status.global.skill?.configured ? 'ready' : 'missing'} — ${status.global.skill?.skillFile ?? '-'}`
    );
  }
  ...
  if (status.projectScope) {
    ...
    console.log(
      `  Skill : ${status.projectScope.skill?.configured ? 'ready' : 'missing'} — ${status.projectScope.skill?.skillFile ?? '-'}`
    );
    console.log(`  Trust : ${status.trustRequired ? 'required' : 'n/a'}`);
  }

  console.log('');
  console.log(`Effective MCP   : ${status.effective.mcp}`);
  console.log(`Effective Hooks : ${status.effective.hooks}`);
  console.log(`Effective Skill : ${status.effective.skill}`);
...
function main(): void {
  ...
  if (!status.installed) {
    throw new Error(
      `Grok Build integration installation did not verify successfully (state=${status.state}).`
    );
  }
  ...
  console.log('✅ Grok Build integration installed');
  ...
  if (scope === 'both') {
    ...
    console.log('Project MCP precedence: project fully replaces global same-name server');
    console.log('Project skill precedence: project > global');
    console.log('Project trust: required by Grok Build');
  }
