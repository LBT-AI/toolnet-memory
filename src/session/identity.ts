...
import type { SessionAgent, SessionIdentity } from './types.js';
...
function safeSegment(value: string, fallback: string): string {
  ...
  const digest = sha256(trimmed).slice(0, 12);

  if (!readable || readable === '.' || readable === '..') {
    return `${fallback}--${digest}`;
  }

  const limited = readable.slice(0, 100);

  /*
   * Preserve normal UUID/OpenCode IDs exactly.
   * Add hash only when sanitization changed the value.
   */
  if (limited === trimmed && trimmed.length <= 100) {
    return limited;
  }

  return `${limited.slice(0, 85)}--${digest}`;
}
...
export function createSessionIdentity(
  ...
  const cleanSessionId = nativeSessionId.trim();
  ...
  const agentFolder = safeSegment(cleanAgent.toLowerCase(), 'agent');

  const sessionFolder = safeSegment(cleanSessionId, 'session');
  ...
  return {
    ...
    nativeSessionId: cleanSessionId,
