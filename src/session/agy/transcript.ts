...
function classify(value: Record<string, unknown>): SessionEventInput['type'] {
  ...
  if (role === 'assistant' || type.includes('assistant') || type.includes('model')) {
    return 'assistant_message';
  }

  if (type.includes('tool') || 'toolCall' in value || 'tool_call' in value) {
    return 'tool_call';
  }
...
function timestampOf(value: Record<string, unknown>): string {
  const candidates = [value.timestamp, value.createdAt, value.time, value.created_at];
...
export function readAgyTranscript(
   ...
   */
  const reset = cursor.offset > size;
  ...
  for (const rawLine of complete.split('\n')) {
    ...
    try {
      ...
      events.push({
        ...
        sourceEventId: `transcript:${absoluteOffset}`,

        sourceSequence: absoluteOffset,
