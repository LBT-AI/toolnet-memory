...
export class AutoContextBuilder {
  ...
  build(projectId: string, query: string, options: AutoContextOptions = {}): string {
    ...
    const rules = this.memory.byType(projectId, 'rule').slice(0, maxRules);

    const todos = this.memory.byType(projectId, 'todo').slice(0, maxTodos);

    const decisions = this.memory.byType(projectId, 'decision').slice(0, maxDecisions);

    const relevant = this.retrieval.search(projectId, query, {
      topK: maxRelevant,
    });

    const architecture = getArchitecture(this.graph, projectId);
    ...
    for (const item of rules) {
      blocks.push(`[RULE] ${item.content}`);
    }

    for (const item of decisions) {
      blocks.push(`[DECISION] ${item.content}`);
    }

    for (const item of todos) {
      blocks.push(`[TODO] ${item.content}`);
    }

    for (const item of relevant) {
      const prefix = item.memory.type.toUpperCase();

      const block = `[${prefix}] ${item.memory.content}`;
    ...
    for (const block of blocks) {
      const next = output.length === 0 ? block : `${output}\n${block}`;
