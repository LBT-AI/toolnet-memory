...
import { reconcileSessionMemoryJournal } from '../session/learner/index.js';
...
export class ToolNetMemoryRuntime {
  ...
  readonly autoRetrieval: AutoRetrieval;

  readonly impactGuard: ImpactGuard;

  readonly autoImpactGuard: AutoImpactGuard;
  ...
  constructor(readonly options: ToolNetMemoryRuntimeOptions) {
    ...
    this.retrieval = new RetrievalEngine(this.memory);

    this.references = new ReferenceResolver(this.graph);
    ...
    const contextBuilder = new AutoContextBuilder(this.memory, this.retrieval, this.graph);

    this.autoRetrieval = new AutoRetrieval(this.hooks, contextBuilder, options.project.id);

    this.impactGuard = new ImpactGuard(this.graph);

    this.autoImpactGuard = new AutoImpactGuard(this.impactGuard, options.project.id);
  }
  ...
  async start() {
     ...
     */
    await reconcileSessionMemoryJournal(project, this.options.storage);

    // Load persistent memory.
    const memories = await this.memoryStore.load(project.id);

    this.memory.importRecords(memories);

    const maintenance = new MemoryMaintenance(this.memory);

    const maintenanceStats = maintenance.run(project.id);

    if (maintenanceStats.expiredRemoved > 0 || maintenanceStats.supersededRemoved > 0) {
      await this.memoryStore.save(project.id, this.memory.exportProject(project.id));
    }

    // Load code graph.
    const graphSnapshot = await this.graphStore.load(project.id);
  ...
  async preparePrompt(prompt: string) {
    return this.autoRetrieval.prepare(prompt);
  }
  ...
  async beforeTool(tool: string, input?: unknown) {
    const impact = await this.autoImpactGuard.beforeTool(tool, input);

    await this.hooks.beforeTool(tool, input);

    return impact;
  }

  async afterTool(tool: string, output?: unknown) {
    return this.hooks.afterTool(tool, output);
  }
  ...
  async stop() {
    ...
    const snapshot = await snapshotManager.create(project.id, 'session-end-auto');

    await snapshotManager.prune(project.id, 10);

    // Flush pending memory events.
    await this.hooks.sessionEnd();

    // Consolidate small activity memories.
    const consolidation = new MemoryConsolidator(this.memory).consolidate(project.id);

    if (consolidation.summaryCreated) {
      await this.memoryStore.save(project.id, this.memory.exportProject(project.id));
    }

    // Incrementally update code graph.
    const codeStats = await new IncrementalRepositoryIndexer(storage).index(
      project.id,
      project.rootPath
    );
    ...
    if (graphSnapshot) {
      this.graph.clearProject(project.id);
    ...
    const result = {
      ...
      memories: this.memory.list(project.id).length,
    };
