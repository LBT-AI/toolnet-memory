...
import { withOptionalRemoteEncryption } from './encrypted-provider.js';
...
function secure(provider: StorageProvider): StorageProvider {
  return withOptionalRemoteEncryption(provider);
}

function fallback(localRoot: string, message: string): StorageProvider {
  console.warn(message);
  return secure(new LocalStorageProvider(localRoot));
}
...
export function createStorageProvider(config: StorageFactoryConfig): StorageProvider {
  ...
  if (config.provider === 'huggingface') {
    ...
    if (hf?.namespace && hf.bucket && hf.accessKeyId && hf.secretAccessKey) {
      return secure(
        new HuggingFaceStorageProvider({
