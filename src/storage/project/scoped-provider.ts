...
import { sanitizeProjectFolder } from './folder.js';

import { mapProjectStoragePath } from './layout.js';
...
export class ProjectScopedStorageProvider implements StorageProvider {
  ...
  constructor(
    ...
    this.folder = sanitizeProjectFolder(projectRemote ?? projectName);

    /*
     * Internal modules may still use the immutable project ID
     * in logical paths.
     */
    this.sourcePrefix = `projects/${projectId}`;

    /*
     * Human-readable physical namespace.
     */
    this.targetPrefix = `projects/${this.folder}`;
  }
  ...
  private async registerProject(): Promise<void> {
    const key = `${this.targetPrefix}/project.json`;

    const now = new Date().toISOString();

    let createdAt = now;

    const existingText = await this.provider.getText(key);
    ...
    if (existingText) {
      ...
      if (typeof existing.id === 'string' && existing.id !== this.projectId) {
        throw new Error(
          [
    ...
    const manifest: RemoteProjectManifest = {
      version: 1,
  ...
  private async ensureRegistered(): Promise<void> {
    if (!this.registryPromise) {
      this.registryPromise = this.registerProject();
    }
  ...
  private key(value: string): string {
    value = mapProjectStoragePath(value);

    if (value === this.sourcePrefix) {
      return this.targetPrefix;
    }

    if (value.startsWith(`${this.sourcePrefix}/`)) {
      return this.targetPrefix + value.slice(this.sourcePrefix.length);
    }
     ...
     */
    if (value === this.targetPrefix || value.startsWith(`${this.targetPrefix}/`)) {
      return value;
    }
  ...
  async put(key: string, data: any, contentType?: string) {
    ...
    return this.provider.put(this.key(key), data, contentType);
  }

  async get(key: string) {
    await this.ensureRegistered();

    return this.provider.get(this.key(key));
  }

  async getText(key: string) {
    await this.ensureRegistered();

    return this.provider.getText(this.key(key));
  }

  async delete(key: string) {
    await this.ensureRegistered();

    return this.provider.delete(this.key(key));
  }

  async exists(key: string): Promise<boolean> {
    await this.ensureRegistered();

    return this.provider.exists(this.key(key));
  }

  async list(prefix: string) {
    await this.ensureRegistered();

    return this.provider.list(this.key(prefix));
  }
}
