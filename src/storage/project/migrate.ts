...
async function main() {
  ...
  const oldPrefix = `projects/${project.id}`;

  const newPrefix = `projects/${folder}`;
  ...
  if (oldPrefix === newPrefix) {
    console.log({
      ...
      migrated: 0,
      folder,
      message: 'Already using named project folder',
    });
  ...
  for (const object of objects) {
    ...
    await storage.put(newKey, data, 'application/json');
