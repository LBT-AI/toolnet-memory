...
describe('Semantic Code Search', () => {
  it('chunks code, indexes vectors and retrieves relevant source', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'toolnet-semantic-'));
...
`
      ...
      const indexed = await new RepositoryIndexer().index('test', repo);
      ...
      expect(first.vectorsIndexed).toBeGreaterThan(0);

      const results = await engine.search('login password authentication', 3);
