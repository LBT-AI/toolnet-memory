...
describe("Layer Detector", () => {
  it("classifies architecture layers", () => {
    ...
    expect(
      byFile.get("src/core/memory.ts"),
    ).toBe("domain");

    expect(
      byFile.get("src/storage/store.ts"),
    ).toBe("infrastructure");

    expect(
      byFile.get("src/runtime/runtime.ts"),
    ).toBe("application");

    expect(
      byFile.get("src/mcp/server.ts"),
    ).toBe("interface");

    expect(
      byFile.get("tests/runtime.test.ts"),
    ).toBe("tests");
  });
});
