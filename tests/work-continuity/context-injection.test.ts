...
function project(): ProjectManifest {
  const root = mkdtempSync(join(tmpdir(), 'toolnet-injection-'));
...
describe('Cross-Agent Context Injection', () => {
  it('pulls the remote canonical Startup Brief into local project cache', async () => {
    const p = project();
...
`
    ...
    const cache = await getStartupBriefForInjection(p, storage);

    expect(cache?.text).toContain('Build continuity layer');

    expect(readFileSync(startupBriefFile(p), 'utf8')).toContain('Continue Phase 4');
  });

  it('Agy injects ephemeral Startup Brief and avoids immediate duplicates', async () => {
    const p = project();
...
`
    ...
    const markers = mkdtempSync(join(tmpdir(), 'toolnet-injection-markers-'));

    roots.push(markers);

    const first = await buildAgyPreInvocationOutput({
      project: p,
  ...
  it('Agy reinjects when an old conversation is resumed after a long pause', async () => {
    ...
    await storage.put(`projects/${p.id}/context/startup.md`, 'MISSION\nResume safely.');

    const markers = mkdtempSync(join(tmpdir(), 'toolnet-resume-'));
    ...
    const resumed = await buildAgyPreInvocationOutput({
      project: p,
...
`
    ...
    expect(JSON.stringify(output)).toContain('Avoid blind continuation');
  });
});
