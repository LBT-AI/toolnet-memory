import { readFileSync } from 'node:fs';

import { describe, expect, it } from 'vitest';

import { certifyCrossAgentContinuity } from '../../src/production/continuity-certify.js';

import { PRODUCTION_PACK_REQUIRED_FILES } from '../../src/production/production-certify.js';

describe('8-agent release contract', () => {
  it('certifies the complete 8-agent continuity ring', async () => {
    const result = await certifyCrossAgentContinuity();

    expect(result.passed).toBe(true);
    expect(result.total).toBe(8);
    expect(result.passedCount).toBe(8);

    expect(result.cases.map((item) => [item.from, item.to])).toEqual([
      ['agy', 'codex'],
      ['codex', 'opencode'],
      ['opencode', 'claude'],
      ['claude', 'kiro'],
      ['kiro', 'cursor'],
      ['cursor', 'copilot'],
      ['copilot', 'grok'],
      ['grok', 'agy'],
    ]);
  });

  it('requires all three new production integrations in npm package', () => {
    expect(PRODUCTION_PACK_REQUIRED_FILES).toEqual(
      expect.arrayContaining([
        'bundle/cursor.js',
        'bundle/cursor-hook.js',
        'bundle/copilot.js',
        'bundle/copilot-hook.js',
        'bundle/grok.js',
        'bundle/grok-hook.js',
      ])
    );
  });

  it('keeps production routes for all 8-agent integration entrypoints', () => {
    const source = readFileSync('bin/toolnet-memory', 'utf8');

    for (const route of [
      'integrate:agy)',
      'integrate:opencode)',
      'integrate:codex)',
      'integrate:claude)',
      'integrate:kiro)',
      'integrate:cursor)',
      'integrate:copilot)',
      'integrate:grok)',
    ]) {
      expect(source).toContain(route);
    }
  });

  it('keeps hook routes for Cursor, Copilot and Grok', () => {
    const source = readFileSync('bin/toolnet-memory', 'utf8');

    for (const route of ['session:cursor-hook)', 'session:copilot-hook)', 'session:grok-hook)']) {
      expect(source).toContain(route);
    }
  });

  it('documents all new integrations in CLI help metadata', () => {
    const source = readFileSync('packages/cli/help.ts', 'utf8');

    for (const command of [
      "name: 'integrate:cursor'",
      "name: 'integrate:copilot'",
      "name: 'integrate:grok'",
    ]) {
      expect(source).toContain(command);
    }
  });
});
