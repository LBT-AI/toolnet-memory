...
function fail(label, detail = '') { failures += 1; console.log(`FAIL  ${label}`); if (detail) console.log(`      ${detail}`); }
function read(file) { if (!existsSync(file)) { fail(`required file: ${file}`); return ''; } return readFileSync(file, 'utf8'); }
function json(file) { const raw = read(file); if (!raw) return undefined; try { return JSON.parse(raw); } catch (error) { fail(`valid JSON: ${file}`, error instanceof Error ? error.message : String(error)); return undefined; } }
function exact(label, actual, expected) { if (actual === expected) { pass(label); return; } fail(label, `expected=${String(expected)} actual=${String(actual)}`); }
function contains(label, text, needle) { if (text.includes(needle)) { pass(label); return; } fail(label, `missing=${needle}`); }
function absent(label, text, needle) { if (!text.includes(needle)) { pass(label); return; } fail(label, `unexpected=${needle}`); }
...
exact('path hash not primary for Git', manifest?.hardening?.projectIdentity?.pathHashPrimaryForGit, false);
exact('silent adoption disabled', manifest?.hardening?.projectIdentity?.silentUnverifiedAdoption, false);
...
exact('dead scaffolds removed manifest', manifest?.hardening?.repositoryTruth?.deadScaffoldsRemoved, true);
exact('unclassified zero-byte source count', manifest?.hardening?.repositoryTruth?.unclassifiedZeroByteSourceFiles, 0);
...
for (const dead of ['src/sync/sync-engine.ts', 'src/mcp/tools/memory-rules.ts', 'src/security/encryption.ts', 'src/retrieval/query-expansion.ts']) {
  if (!existsSync(dead)) { pass(`dead scaffold absent: ${dead}`); } else { fail(`dead scaffold absent: ${dead}`); }
}
console.log('');
...
exact('runtime capability docs', manifest?.hardening?.documentationTruth?.runtimeCapabilityMatrix, true);
...
contains('runtime truth cross-machine identity', runtimeTruth, 'normalized Git repository identity');
...
exact('embedding conflict matching disabled', manifest?.hardening?.conflictEngineV2?.embeddingMatching, false);
for (const kind of ['rule', 'decision', 'todo', 'next_action', 'fix', 'context', 'architecture']) {
  if (manifest?.hardening?.conflictEngineV2?.kinds?.includes(kind)) { pass(`conflict kind: ${kind}`); } else { fail(`conflict kind: ${kind}`); }
}
...
for (const file of [
  ...
  if (existsSync(file)) { pass(`phase test exists: ${file}`); } else { fail(`phase test exists: ${file}`); }
}
for (const audit of [
...
absent('no encryption master key contract', projectIdentity + projectRegistry + conflict, 'TOOLNET_MEMORY_MASTER_KEY');
absent('no embedding provider introduced', projectIdentity + projectRegistry + conflict, 'EmbeddingProvider');
console.log('');
console.log('=== STORAGE FREEZE ===');
const storage = spawnSync('git', ['status', '--porcelain', '--', 'src/storage'], { encoding: 'utf8' });
if (storage.status === 0 && !storage.stdout.trim()) { pass('src/storage unchanged'); } else { fail('src/storage unchanged', (storage.stdout || storage.stderr || '').trim()); }
console.log('');
console.log('======================================');
if (failures === 0) { console.log('V0316_FEATURE_CONTRACT=PASS'); process.exitCode = 0; } else { console.log(`V0316_FEATURE_CONTRACT=FAIL failures=${failures}`); process.exitCode = 1; }
