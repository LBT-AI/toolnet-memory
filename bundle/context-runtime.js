...
`)}function Oe(r,t){let{maxTokens:e,trimMarker:n="[Context trimmed by ToolNet Memory token budget]"}=t;if(r.length===0)return"";let s=[...r].sort((i,l)=>l.priority-i.priority),o=[],a=0,d=!1;for(let i of s){let l=`# ${i.title}
...
`;return l+f+k+g+Me}function ie(r={}){let t=r.projectPath||process.cwd(),e=x(t);if(!e)throw new Error("Not in a ToolNet project (no .toolnet directory found)");let n=m.join(e,".toolnet","profile.md"),s=m.join(e,".toolnet","current.md"),o=T(n)||"",a=T(s)||"",d=`# AI Startup Instructions
