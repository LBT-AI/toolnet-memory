import*as f from"fs";import*as d from"path";function v(e){return e?Math.ceil(e.length/3.5):0}function Q(e,n){if(!e)return"";if(v(e)<=n)return e;let r=Math.floor(n*3.5),o=e.slice(0,r),s=o.lastIndexOf("."),i=o.lastIndexOf(`
`),a=Math.max(s,i);return a>r*.7?o.slice(0,a+1):o}function k(e,n){if(!e)return"";let t=e.split(`
...
`.trim()}function ae(e){let n=d.resolve(e),t=d.parse(n).root;for(;n!==t;){let r=d.join(n,".toolnet");if(f.existsSync(r)&&f.statSync(r).isDirectory())return n;n=d.dirname(n)}return null}function V(e){try{return f.existsSync(e)?f.readFileSync(e,"utf-8").trim():null}catch{return null}}function z(e){return e.split(`
...
`)}function G(e={}){let n=e.projectPath||process.cwd(),t=ae(n);if(!t)return null;let r=d.join(t,".toolnet","project.json"),o="Unknown";try{if(f.existsSync(r)){let M=JSON.parse(f.readFileSync(r,"utf-8"));o=M.name||M.projectName||"Unknown"}}catch{}let s=d.join(t,".toolnet","profile.md"),i=d.join(t,".toolnet","current.md"),a=V(s)||"",c=V(i)||"";a=z(a),c=z(c);let u=`[TOOLNET PROJECT CONTEXT]

Project: ${o}
Root: ${t}

`,p=T(a,c),l=L({id:"",name:o,rootPath:t},1600),h=l?`
...
`)}async function Ne(){let e=Date.now(),n=await _e();if(n.hook_event_name!=="SessionStart"){m();return}let t=typeof n.cwd=="string"?n.cwd:"";if(!t){m();return}if(!W(t)){m();return}try{let o=G({projectPath:t});if(!o?.trim()){m();return}let s=Ie(o),i={hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:s}};je(e,t,s.length),process.stdout.write(JSON.stringify(i))}catch{m()}}Ne().catch(()=>{m(),process.exitCode=0});
