...
`),{installed:!0,pluginRoot:n,files:[r,s,i,c]}}import{existsSync as vr,mkdirSync as Wt,readFileSync as jr,writeFileSync as zt}from"node:fs";import{join as Ut}from"node:path";var wr="memory_agent_ask";function Bt(){return`
...
`.trim()}var Yt="<!-- TOOLNET_MEMORY_BOOTSTRAP_START -->",Fe="<!-- TOOLNET_MEMORY_BOOTSTRAP_END -->";function Or(e={}){let t=dt();Wt(x(),{recursive:!0});let o=`${Yt}
## ToolNet Memory
...
${Fe}`,n=vr(t)?jr(t,"utf8"):"",r=n.indexOf(Yt),s=n.indexOf(Fe);return r>=0&&s>=r?n=n.slice(0,r)+o+n.slice(s+Fe.length):(n=n.trimEnd(),n&&(n+=`

`),n+=o),zt(t,n.trimEnd()+`
`,{encoding:"utf8",mode:384}),t}function qt(e={}){let t=e.binary??process.env.TOOLNET_MEMORY_BIN??"toolnet-memory",o=[];o.push(Or({cwd:e.cwd}));let n=e.scope??"global",r=[];if((n==="global"||n==="both")&&r.push(e.directory??pt()),n==="project"||n==="both"){let s=e.cwd??process.cwd();r.push(Ut(s,".opencode","plugins"))}for(let s of r){Wt(s,{recursive:!0});let i=Ut(s,"toolnet-memory.js"),c=`
...
Current repository evidence overrides stale memory.
`;function Os(e,t){bs(js(e),{recursive:!0,mode:448});let o=`${e}.toolnet-${process.pid}-${Date.now()}.tmp`;try{vs(o,t,{encoding:"utf8",mode:384}),Cs(o,e)}finally{ws(o,{force:!0})}}function ze(e={}){let t=e.skillFile??ie();if(ks(t)&&Yo(t,"utf8")===We)return{skillFile:t,changed:!1};if(Os(t,We),Yo(t,"utf8")!==We)throw new Error("Grok ToolNet continuity skill was written but verification failed.");return{skillFile:t,changed:!0}}var K=[["SessionStart",10],["UserPromptSubmit",10],["PreToolUse",10],["PostToolUse",15],["Stop",30]];function Wo(e){return!g(e)||!Array.isArray(e.hooks)?!1:e.hooks.some(t=>g(t)&&typeof t.command=="string"&&t.command.includes("session:grok-hook"))}function Is(e,t,o){let n={hooks:[{type:"command",command:`${t} session:grok-hook`,timeout:o,env:{TOOLNET_HOOK_EVENT:e}}]};return e==="PreToolUse"&&(n.matcher=".*"),n}function qe(e={}){let t=e.hooksFile??re(),o=e.binary??process.env.TOOLNET_MEMORY_BIN??"toolnet-memory",n=b(t,"Grok Build"),r=n.hooks;if(r!==void 0&&!g(r))throw new Error("Invalid existing Grok Build hooks file: hooks must be an object.");let s=g(r)?{...r}:{};for(let[l,u]of K){let p=s[l];if(p!==void 0&&!Array.isArray(p))throw new Error(`Invalid existing Grok Build hooks file: hooks.${l} must be an array.`);let d=Array.isArray(p)?p.filter(P=>!Wo(P)):[];s[l]=[...d,Is(l,o,u)]}let i={...n,hooks:s};if(JSON.stringify(n)===JSON.stringify(i))return{hooksFile:t,changed:!1,hookCount:K.length};T(t,i);let c=b(t,"Grok Build");if(!g(c.hooks))throw new Error("Grok Build hooks were written but verification failed.");let a=0;for(let[l]of K){let u=c.hooks[l];if(!Array.isArray(u))throw new Error("Grok Build hooks were written but verification failed.");a+=u.filter(Wo).length}if(a!==K.length)throw new Error("Grok Build hooks were written but verification failed.");return{hooksFile:t,changed:!0,hookCount:K.length}}import{existsSync as Ss,mkdirSync as xs,readFileSync as Rs,renameSync as Ms,rmSync as Ts,writeFileSync as Es}from"node:fs";import{dirname as Fs}from"node:path";function zo(e){return Ss(e)?Rs(e,"utf8"):""}function Ps(e,t){xs(Fs(e),{recursive:!0,mode:448});let o=`${e}.tmp-${process.pid}-${Date.now()}`;try{Es(o,t,{encoding:"utf8",mode:384}),Ms(o,e)}finally{Ts(o,{force:!0})}}function Ve(e){return e.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/\t/g,"\\t")}function As(e){return`[mcp_servers."${Ve(e)}"]`}function Ns(e,t){return[As(e),`command = "${Ve(t)}"`,'args = ["mcp"]',"enabled = true"].join(`
